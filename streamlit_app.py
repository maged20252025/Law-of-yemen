    ...
    if st.session_state.search_done and st.session_state.results:
        ...
        if st.button("📄 تحميل النتائج كملف Word"):
            ...
            for r in filtered_results:
                ...
                full_text = f"{r['law']} - المادة {r['num']}\n"
                p.add_run(full_text).bold = True
                base_text = r["text"]
                for word in re.split(r'(\s+)', base_text):
                    run = p.add_run(word)
                    if any(kw.lower() in word.lower() for kw in kw_list):
                        run.font.highlight_color = 3  # Yellow
                    run.font.size = Pt(14)
                docx_out.add_paragraph("")
            buffer = BytesIO()
            docx_out.save(buffer)
            st.download_button("⬇️ تحميل ملف Word", buffer.getvalue(), file_name="نتائج_القوانين.docx")

        # زر تحميل PDF
        if st.button("📝 تحميل النتائج كـ PDF"):
            buffer_pdf = BytesIO()
            c = canvas.Canvas(buffer_pdf, pagesize=A4)
            width, height = A4
            y = height - 2 * cm
            for r in filtered_results:
                line = f"{r['law']} - المادة {r['num']}: {r['text']}"
                lines = re.findall(r'.{1,100}(?:\s|$)', line)
                for l in lines:
                    c.drawRightString(width - 2 * cm, y, l.strip())
                    y -= 14
                    if y < 2 * cm:
                        c.showPage()
                        y = height - 2 * cm
                y -= 10
            c.save()
            st.download_button("⬇️ تحميل ملف PDF", buffer_pdf.getvalue(), file_name="نتائج_القوانين.pdf")
