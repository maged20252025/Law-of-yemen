...
def run_main_app():
    ...
    if "search_done" not in st.session_state:
        st.session_state.get("search_done") = False
    ...
    if st.button("🔍 بدء البحث") and keywords:
        ...
        st.session_state.get("results") = results
        st.session_state.get("search_done") = True

    if st.session_state.get("search_done") and st.session_state.get("results"):
        ...
        for r in filtered_results:
            with st.container():
                st.markdown(f"📘 **القانون:** {r['law']}")
                st.markdown(f"🔢 **المادة رقم:** {r['num']}")
                st.code(r['text'], language='text')

        export_option = st.radio("اختر البيانات التي تريد تصديرها:", ["جميع النتائج", "فقط النتائج الظاهرة بعد التصفية"])
        results_to_export = results if export_option == "جميع النتائج" else filtered_results

        if st.button("📥 تصدير النتائج إلى ملف Word"):
            doc = Document()
            doc.add_heading("نتائج البحث في القوانين اليمنية", level=1)

            for item in results_to_export:
                doc.add_paragraph(f"📘 القانون: {item['law']}", style='Heading 2')
                doc.add_paragraph(f"🔢 المادة رقم: {item['num']}", style='Heading 3')
