...
def run_main_app():
    ...
    if st.session_state.search_done and st.session_state.results:
        ...
        for r in filtered_results:
            st.markdown(f"""<div style='background-color:#f1f8e9;padding:15px;margin-bottom:15px;border-radius:10px;
...
# ========== تصدير النتائج إلى ملف Word ==========
if st.session_state.search_done and st.session_state.results:
    export_option = st.radio("اختر البيانات التي تريد تصديرها:", ["جميع النتائج", "فقط النتائج الظاهرة بعد التصفية"])
    results_to_export = st.session_state.results if export_option == "جميع النتائج" else filtered_results
